...
func (s *EnsureCleanOrSaveStep) Execute(ctx context.Context) error {
	...
	if s.AssumeYes {
		return errors.New("cannot auto-squash with dirty directory in non-interactive mode")
	}
...
func (s *SoftResetStep) Execute(ctx context.Context) error {
	if s.State.CommitCount > 1 {
		if err := s.GitClient.ResetSoft(ctx, s.State.MergeBase); err != nil {
	...
	if !confirm {
		return errors.New("squash aborted by user (low commit count)")
	}
...
func (s *CommitSquashStep) Execute(ctx context.Context) error {
	...
	if err := s.GitClient.Commit(ctx, message); err != nil {
		return fmt.Errorf("failed to commit: %w", err)
	}
...
func (s *ForcePushStep) Execute(ctx context.Context) error {
	...
	if err := s.GitClient.ForcePushLease(ctx, s.State.CurrentBranch); err != nil {
